...
export class GoogleStrategy extends PassportStrategy(Strategy, 'google') {
  /**
   * Input: ConfigService chứa GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, FRONTEND_ORIGIN.
   * Output: Khởi tạo Google OAuth strategy cho Passport, callback chạy qua FE để cookie thuộc FE host.
   */
  constructor(configService: ConfigService) {
    super({
      ...
      callbackURL: `${getRequiredConfig(configService, 'FRONTEND_ORIGIN', ERROR_CODES.SYS_004)}/auth/google/callback`,
      scope: ['email', 'profile'],
    });
  }
