...
- Redis: store validate session tốc độ cao cho gateway và cache layer dùng cùng core.

Vai trò hai service backend:
- `api-gateway/`: điểm vào public duy nhất (port `8000`), xử lý CORS/CSRF + xác thực edge bằng Redis rồi proxy sang core.
- `core/`: service nội bộ (port `8001`, đổi tên từ `api/`), xử lý OAuth Google, phiên, device/account; tin cậy identity header do gateway inject.
...
|---|---|---|
| `core/.prettierrc` | Cấu hình | Thiết lập format code cho backend. |
| `core/eslint.config.mjs` | Cấu hình | Thiết lập luật lint cho backend. |
| `core/nest-cli.json` | Cấu hình | Cấu hình Nest CLI cho build/generate. |
| `core/package.json` | Metadata/Script | Khai báo script và dependency backend, gồm nhóm lệnh database với tiền tố `db:` (`db:generate`, `db:validate`, `db:migrate:*`, `db:push`, `db:pull`, `db:studio`). |
| `core/pnpm-lock.yaml` | Lockfile | Khóa phiên bản dependency backend. |
| `core/prisma/schema.prisma` | Cấu hình database | Định nghĩa schema Prisma cho PostgreSQL. |
| `core/prisma.config.ts` | Cấu hình database | Cấu hình Prisma CLI và datasource URL từ môi trường. |
| `core/src/main.ts` | Mã nguồn | Entry point khởi tạo ứng dụng NestJS, chạy HTTP server nội bộ ở `PORT=8001`. CORS/CSRF đã chuyển hết về API Gateway nên core không tự bật nữa. |
...
| `core/src/common/constants/error-codes.constant.ts` | Mã nguồn | Danh sách mã lỗi chuẩn dùng chung toàn backend. |
| `core/src/common/exceptions/app.exception.ts` | Mã nguồn | Exception nghiệp vụ dùng object mã lỗi chuẩn và HTTP status tương ứng. |
| `core/src/common/filters/http-exception.filter.ts` | Mã nguồn | Global filter map exception về format lỗi chuẩn của dự án. |
| `core/src/common/guards/google-auth.guard.ts` | Mã nguồn | Guard bọc `AuthGuard('google')` để kích hoạt luồng OAuth Google. |
| `core/src/common/interfaces/express-user.d.ts` | Khai báo kiểu | Mở rộng type `Express.User` dùng chung cho `req.user`. |
| `core/src/common/interceptors/response.interceptor.ts` | Mã nguồn | Global interceptor chuẩn hóa success response. |
| `core/src/common/loggers/app.logger.ts` | Mã nguồn | Custom logger dùng queue bất đồng bộ cho log output, vẫn in terminal và ghi thêm vào `core/logs/YYYY-MM-DD.log`, đồng thời rút gọn log lỗi bootstrap/runtime. |
| `core/src/common/pipes/parse-uuid.pipe.ts` | Mã nguồn | Pipe UUID dùng chung với format lỗi thống nhất. |
| `core/src/common/strategies/google.strategy.ts` | Mã nguồn | Passport strategy xác thực Google OAuth 2.0 dùng chung. |
| `core/src/common/utils/functions.ts` | Mã nguồn | File tập trung các hàm dùng chung backend, gồm hàm đọc cấu hình bắt buộc và helper xác định runtime production theo `NODE_ENV`. |
| `core/src/common/utils/types.ts` | Mã nguồn | File tập trung các type dùng chung backend như error code, response envelope và Google user profile. |
| `core/src/database/database.module.ts` | Mã nguồn | Module đóng gói và export service kết nối database. |
| `core/src/database/database.service.ts` | Mã nguồn | Service Prisma dùng chung để các module khác inject. |
| `core/tsconfig.json` | Cấu hình | Cấu hình TypeScript chính của backend. |
| `core/tsconfig.build.json` | Cấu hình build | Cấu hình TypeScript cho quá trình build. |
...
| Đường dẫn | Loại | Vai trò |
|---|---|---|
| `api-gateway/.env.example` | Cấu hình | Mẫu biến môi trường gateway (`PORT=8000`, `CORE_URL`, `REDIS_*`, `CORS_ALLOWED_ORIGINS`, `COOKIE_DOMAIN`). |
